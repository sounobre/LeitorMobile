...
- Status: `IN_PROGRESS`; API 37 runtime remains RED. Branch `fix/mobile-startup-update-depth`, based on `origin/main` `106cc37e9cd13d13b5705d680eb63797657a0a29`. Physical short worktree: `D:\LM5C`.
- Runtime RED: `Pixel_8`, `emulator-5554`, API 37 / Android 17 / x86_64. Metro loaded `expo-router/entry.js`; React Native then reported `Maximum update depth exceeded`. The component stack points to Expo Router `Content` / `useNavigationBuilder` (`node_modules/expo-router/build/ExpoRoot.js:145`). Android hierarchy showed `Render Error` before login; process stayed alive; product controls were absent. No login was reached.
- Initial hypothesis: `useRouter()` object identity caused repeated auth checks. The real `RootLayout` harness proved this component-level defect: stable db/route with a fresh router object triggered 2 `getSession` calls instead of 1, one `replace('/login')`, and no `Stack` render. This hypothesis is confirmed in isolation but is not the sole Android runtime cause.
- Runtime diagnosis used temporary, uncommitted logs in `RootLayout` / `AuthGate`: RootLayout rendered once; AuthGate rendered 51 times and started 50 auth effects. `db` stayed identity 1 and `router` identity 2, while `inLogin` repeatedly alternated `false` / `true`; all session results were absent. The `ready` condition removes `Stack` when `inLogin` changes. Removing the navigator resets Expo Router to its initial route, which alternates back into the login transition and repeatedly re-enters `Content`. This semantic-route / conditional-Stack-unmount cycle is the confirmed runtime trigger. Temporary instrumentation was reverted.
- A first minimal experiment that removed router identity from the effect/readiness key passed the focused identity test but still reproduced the Android runtime error; that incomplete experiment was reverted before investigating the route/unmount cycle.
- Regression test RED: `src/components/authGate.test.ts` now checks both router identity churn and semantic route change. Focused Jest currently reports 2 tests / 2 failures against production: identity churn causes a second auth lookup; the semantic route change does start a second lookup but removes `Stack` while that lookup is pending. This preserves evidence before the production fix.
- Baseline at the requested base: `npm ci --prefer-offline` completed; `package.json` and `package-lock.json` unchanged; typecheck passed; lint had 0 errors / 14 warnings; Jest passed 12 suites / 107 tests.
- Native build: temporary `android/` prebuild without `--clean`; x86_64 `:app:assembleDebug` succeeded after a transient file-lock failure and one retry. APK: `D:\LM5C\leitor-epub\android\app\build\outputs\apk\debug\app-debug.apk` (148,923,547 bytes). Package `com.example.leitorepub` installed and launched.
- Metro loaded the instrumented bundle over `adb reverse tcp:8081 tcp:8081` using the process-only `NODE_OPTIONS=--use-system-ca` workaround and the Wave 5B dev-client deep link. This workaround is not a project change. Temporary runtime logs, screenshot and hierarchy are under `%TEMP%\wave-5c-runtime\`.
- Production fix, component GREEN, final JS regression, Android post-fix runtime, login UI: `PENDING`. `android/` and APK remain ignored / uncommitted.
- TEST IDs executed: **NONE**. All 17 Wave 5 IDs remain `NOT_RUN`; TEST-MATRIX unchanged. Appium remains unselected; no Maestro/Detox installed; API 29 not started.