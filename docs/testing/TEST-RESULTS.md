...
## Wave 5B — Appium practical smoke

- Status: `TOOL_SPIKE_INCOMPLETE`; tool decision remains `UNRESOLVED`. Appium UiAutomator2 is still the first candidate, not yet selected or rejected. The device/build/harness gates advanced, but the product's React Native startup stopped before a usable screen.
- Base: `origin/main` `983df3c487d95eb57923ba90dbf5bc9bfedf3d7d`; isolated branch `test/wave-5b-appium-smoke`. Windows NT 10.0.26200.0; Node v26.8.1; npm 11.19.0; Java 21.0.6; Android SDK `C:\Users\souno\AppData\Local\Android\Sdk`; adb 1.0.41 / platform-tools 37.0.0.
- Target: `Pixel_8` AVD booted (`sys.boot_completed=1`), `emulator-5554`, Android API 37 / release 17 / x86_64. API 29: `NOT_RUN`; no image installed.
- Build: Expo prebuild generated temporary `android/` without `--clean`. Long Windows worktree path caused Ninja/CMake dirty-manifest failures; physical short detached build worktree `D:\LM5B` produced `:app:assembleDebug` x86_64 successfully. APK `D:\LM5B\leitor-epub\android\app\build\outputs\apk\debug\app-debug.apk`; package `com.example.leitorepub`, launcher `com.example.leitorepub.MainActivity`. `adb install -r` succeeded, and Android launched a live process without native crash. Generated `android/` and APK are not committed.
- Development runtime: Metro loaded `expo-router/entry.js` after a process-only Windows CA fix and localhost deep link over `adb reverse`. The app then showed React Native `Render Error: Maximum update depth exceeded` before login. Stack evidence names ReactFabric / Expo Router but does not prove a unique cause. Classification: `APP_RUNTIME_BLOCKED`, not Appium failure. No production edit was made.
- Appium: external Appium `3.7.0`, UiAutomator2 `8.7.0`, required doctor checks passed, server `/status` ready. W3C HTTP client was temporary PowerShell. Two sessions were created; the first expired at the default 60-second idle timeout and the second used 300 seconds. Foreground package `com.example.leitorepub`; Appium returned XML and screenshot. Real overlay selectors included `text="Render Error"` / class `android.widget.TextView` and `content-desc="Dismiss"` / class `android.view.ViewGroup`; `resource-id` was empty. Locating and clicking `Dismiss` worked, but the error persisted. Product RN control and Expo Router navigation: `NOT_PROVEN`.
- External UI: Android `ACTION_OPEN_DOCUMENT` launched the Picker directly; Appium saw package `com.google.android.documentsui`, captured XML and screenshot, and stayed usable after Back returned to the app. This did not invoke the app's import flow and did not import an EPUB. Reader unavailable; contexts `NATIVE_APP` only; WebView/Chromedriver, share and browser: `NOT_PROVEN`.
- Artifacts: `%TEMP%\wave-5b-appium-smoke\appium-screenshot.png`, `appium-page-source.xml`, `appium-picker-screenshot.png`, `appium-picker-source.xml`, `appium-server.log`, `logcat-filtered.txt`, plus build/Metro logs. They are temporary and not committed. Detailed command/evidence chain is in `MOBILE-E2E-TOOL-DECISION.md`.
- TEST IDs executed: **NONE**. TEST-004, TEST-011, TEST-017, TEST-019, TEST-022, TEST-025, TEST-030, TEST-032, TEST-033, TEST-034, TEST-036, TEST-038, TEST-043, TEST-048, TEST-051, TEST-052 and TEST-053 remain `NOT_RUN`. TEST-MATRIX unchanged.
- Production files changed: `NONE`. package.json changed: `NO`. package-lock.json changed: `NO`. Appium/driver were installed outside the repo; no project dependency changed.
- Recommendation: `NEXT: RESOLVE APP RUNTIME BLOCKER`; then repeat the product RN interaction/navigation and Reader or app-triggered Picker gate before selecting a tool or starting Wave 5 test implementation.

## Wave 5C — Mobile startup runtime blocker

- Status: `PASS`; `APP_RUNTIME_BLOCKER: FIXED`. Branch `fix/mobile-startup-update-depth`, based on `origin/main` `106cc37e9cd13d13b5705d680eb63797657a0a29`; short physical worktree `D:\LM5C`.
- Runtime RED: the original Pixel_8 / `emulator-5554` API 37 run loaded `expo-router/entry.js`, then showed React Native `Maximum update depth exceeded` before login. Error hierarchy showed `Render Error` at Expo Router `Content` / `useNavigationBuilder` (`node_modules/expo-router/build/ExpoRoot.js:145`). The app process stayed alive but no product controls appeared.
- Root-cause investigation: `b33fc8aa2a486c8c22bab9116e6ef21a7cdc0ac6` made router object identity part of `AuthGate` readiness and auth-effect dependencies. The RootLayout harness confirmed a real isolated defect: a new `useRouter()` object with stable db and route triggered 2 `getSession` calls instead of 1, one `replace('/login')`, and no Stack render. However, removing router identity alone still reproduced the Android error; that experiment was reverted.
- Confirmed Android trigger: temporary runtime logs showed RootLayout render `1`, AuthGate renders `51`, auth effect starts `50`; db identity remained `1`, router identity remained `2`, but `inLogin` repeatedly alternated `false` / `true` with no session. When the route changed, `ready` became false and AuthGate removed its `<Stack>`. The navigator then reset to its initial route, causing another semantic route transition and repeating auth/navigation updates until Expo Router `Content` hit maximum update depth.
- RED test: `src/components/authGate.test.ts` rendered the real RootLayout with pass-through provider mocks and a fresh router object each render. Before the fix, focused Jest had 2 tests / 2 failures: router identity churn caused an extra auth lookup; a semantic route change correctly started another lookup but removed Stack while that lookup was pending. The RED run also observed `replace('/login')` once.
- Production fix: only `leitor-epub/app/_layout.tsx`. The auth effect depends on `[db, inLogin]`; router identity is held in a ref for imperative redirects. `lastAuthCheck` is keyed by `db` and `inLogin`, not router. The initial activity indicator remains until the first check for the db completes; after that, Stack stays mounted while a route-specific check runs. The cancellation guard remains. Redirects remain `no session + outside login -> /login` and `session + in login -> /`, with no redirect when already on the matching route.
- Component GREEN: focused AuthGate test passed 2/2. Router identity churn resulted in 1 auth lookup; `inLogin` change triggered another lookup while Stack continued rendering. The harness produced no maximum-depth loop.
- JavaScript regression: `npm run typecheck` passed; `npm run lint` passed with 0 errors and 14 existing warnings; `npm test -- --runInBand` passed 13 suites / 109 tests (the original 12 suites / 107 tests plus the 2 new tests).
- Android GREEN: x86_64 `:app:assembleDebug` succeeded in the short worktree; APK `D:\LM5C\leitor-epub\android\app\build\outputs\apk\debug\app-debug.apk` (148,923,547 bytes). `adb install -r` succeeded. The installed development build was cold-launched against the post-fix Metro bundle over `adb reverse tcp:8081 tcp:8081` with the process-only `NODE_OPTIONS=--use-system-ca` workaround. Target: Pixel_8, `emulator-5554`, API 37 / Android 17 / x86_64; process PID `6044` remained alive. Fresh logcat: 0 `Maximum update depth`, 0 `Render Error`, 0 `FATAL EXCEPTION`. UI hierarchy contained `Entrar no Leitor`, `E-mail`, `Senha`, and `Endereço da API`; no login was performed.
- Runtime evidence: `%TEMP%\wave-5c-runtime\green-screen.png`, `green-hierarchy.xml`, `green-logcat.txt`, and Metro stdout/stderr logs. Initial RED evidence and diagnostic hierarchy/logs are also retained in the same temporary directory. Generated `android/` remains ignored and uncommitted. The JS-only fix was served by Metro; no native dependency or project manifest changed.
- Baseline and manifests: `npm ci --prefer-offline` completed; `package.json` and `package-lock.json` unchanged. No dependency was added. TEST-MATRIX unchanged.
- TEST IDs executed: **NONE**. All 17 Wave 5 IDs remain `NOT_RUN`. Appium remains unselected; Maestro/Detox were not installed; API 29 was not started.
- Recommendation: `READY TO REPEAT APPIUM SMOKE`. This wave fixes the startup blocker only; it does not select Appium or execute a Wave 5 TEST ID.

## Wave 5D — Appium product smoke

- Status: `TOOL_SELECTED`; selected harness: **Appium UiAutomator2**. This supersedes the provisional decision in earlier waves. Base: `origin/main` `b51fb5ae12172705e62f482e0ccfdcfb94c5ad6f`; branch `test/wave-5d-appium-product-smoke`; worktree `D:\LM5D`.
- Target/build: Pixel_8 / `emulator-5554` / API 37 / x86_64. Debug APK `D:\LM5D\leitor-epub\android\app\build\outputs\apk\debug\app-debug.apk`, 148,923,547 bytes; native build and install succeeded. Metro loaded `expo-router/entry.js`; app process remained alive. No `Maximum update depth exceeded`, `Render Error` or `FATAL EXCEPTION` was found. One adb/UiAutomator2 stall was recovered by restarting the same AVD without wipe; a new Appium session then completed all gates.
- Appium `3.7.0`; UiAutomator2 `8.7.0`; successful session used the requested Android/device/package/activity capabilities and stayed valid through system-picker cancel. Appium observed the login hierarchy and changed the email, password and API address inputs. Actual selectors are recorded in `MOBILE-E2E-TOOL-DECISION.md`; no synthetic `testID` was assumed.
- A temporary fake API received exactly `POST /api/auth/login`, `GET /api/books` and `GET /api/cards?includeArchived=true`, with no unexpected requests. Synthetic login reached `Leitor EPUB` / `Sua biblioteca está vazia`. Appium navigated `/` → `/cards` (`Cards`, `Fila vazia`) → `/` through the product menu. The product’s `Importar livro EPUB` control opened `com.google.android.documentsui`; Appium observed `Recent files`, `Search` and `Show roots`, then used Back to return to the still-empty library with the session valid.
- Reader WebView: `NOT_PROVEN`; no EPUB was imported. API 29: `PENDING` (not installed or started). Before Group B / TEST-025, run the explicit Reader/WebView compatibility spike.
